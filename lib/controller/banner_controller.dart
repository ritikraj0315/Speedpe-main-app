...
class BannerController extends GetxController implements GetxService {
  ...
  List<BannerModel> getFilteredBannersList(String type) {
    return bannerList
            ?.where(
                (banner) => banner.receiver?.toString().contains(type) ?? false)
            .toList() ??
        [];
  }

  List<BannerModel> getHomeBanners(String type) {
    List<BannerModel> filteredList = getFilteredBannersList(type);
