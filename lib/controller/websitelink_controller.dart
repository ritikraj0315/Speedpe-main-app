...
class WebsiteLinkController extends GetxController implements GetxService {
  ...
  List<WebsiteLinkModel> getFilteredWebsiteList(String category) {
    return websiteList
            ?.where((website) =>
  ...
  List<WebsiteLinkModel> getReversedFilteredWebsiteList(String category) {
    List<WebsiteLinkModel> filteredList = getFilteredWebsiteList(category);
    return filteredList.reversed.toList();
  }

  List<WebsiteLinkModel> getExcludedWebsiteList(String category) {
    return websiteList!
        .where((website) =>
            !(website.category?.toLowerCase().contains(category) ?? false))
        .toList();
  ...
  List<WebsiteLinkModel> getSearchWebsiteList(String query) {
    List<WebsiteLinkModel> filteredList = getExcludedWebsiteList("ad");
    return filteredList
            .where((website) =>
